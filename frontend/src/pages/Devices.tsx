  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3, alignItems: 'center' }}>
        <Typography variant="h4" fontWeight="bold">{t('devices.title')}</Typography>
        <Button
          variant="contained"
          startIcon={<AddIcon />}
          onClick={() => handleOpen()}
              ...
              <Card>
                <CardContent>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', mb: 2 }}>
                    <Box>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {device.brand_name || device.custom_brand || '-'} {device.model_name || device.custom_model || ''}
                      </Typography>
                    </Box>
                    <Box>
                  ...
                  {device.next_maintenance_date && (
                    <Chip
                      label={`Next: ${new Date(device.next_maintenance_date).toLocaleDateString()}`}
                      size="small"
                      color={new Date(device.next_maintenance_date) < new Date() ? 'error' : 'primary'}
                      sx={{ mt: 1 }}
                    />
                  )}
                  {device.client_name && (
